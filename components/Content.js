...
function ContentItem({ image, title, tag, date }) {
    ...
    return (
        <div class="w-[286px]">
                <div class="h-48 rounded-xl overflow-hidden bg-slate-200 drop-shadow-xl">
                    <img src={`${addImage}`} class="h-full w-full object-cover" alt="Gambar" />
                </div>
                <p class="font-bold text-slate-700 leading-relaxed pt-3">{title}</p>
                <p class="text-xs text-slate-400 pt-2">{tag} / {date}</p>
            </div>
    )
}
...
function ContentItemGroup() {
    return (
        <section class="grid grid-cols-4 grid-rows-2 gap-x-8 gap-y-10 pt-16">
            <ContentItem
